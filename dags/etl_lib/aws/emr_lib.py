...
def create_cluster(region_name, cluster_name='Airflow-POC-' + str(datetime.now()), 
release_label='emr-5.29.0', master_instance_type='m5.xlarge', num_core_nodes=2, 
core_node_instance_type='m5.2xlarge'):
    emr_master_security_group_id = get_security_group_id(
        'airflow-emr', region_name=region_name)
    ...
    cluster_response = emr.run_job_flow(
        ...
        LogUri='s3://pchenni-airflow-poc/s3_emr/logs/'
    )
...
# Creates an interactive pyspark session.
...
def create_spark_session(master_dns, kind='pyspark'):
    ...
    response = requests.post(
        host + '/sessions', data=json.dumps(data), headers=headers)
...
def wait_for_idle_session(master_dns, response_headers):
    ...
    while status != 'idle':
        ...
        try:
            ...
            status_response = session.get(session_url)
            # print(status_response.content)
        except Exception as e:
            print('Exception', session_url)
...
# Submits the pyspark code as a simple JSON command to the Livy server
def submit_statement(session_url, statement_path):
    statements_url = session_url + '/statements'
    with open(statement_path, 'r') as f:
        code = f.read()
    data = {'code': code}
    # data = {'file': statement_path}
    response = requests.post(statements_url, data=json.dumps(
        data), headers={'Content-Type': 'application/json'})
    logging.info(response.json())
    return response


# Function to help track the progress of the python code submitted to Apache Livy
def track_statement_progress(master_dns, response_headers):
    statement_status = ''
    host = 'http://' + master_dns + ':8998'
    session_url = host + \
        response_headers['location'].split('/statements', 1)[0]
    ...
    while statement_status != 'available':
        ...
        # logging the logs
        lines = requests.get(
            session_url + '/log', headers={'Content-Type': 'application/json'}).json()['log']
        for line in lines:
            logging.info(line)
